...
bool KCPWrapper::init(const std::string& multicastIP, uint16_t port, uint32_t conv, bool isReceiver) {
    // 创建UDP socket
    ...
    if (mIsReceiver) {
        std::cout << "receiver bind!" << std::endl;
        // 接收端需要绑定地址
...
int KCPWrapper::udp_output(const char* buf, int len, ikcpcb* kcp, void* user) {
    ...
    std::cout << "udp_output!!!!!!!!!!!!!!!!!!!!!" << std::endl;
    KCPWrapper* wrapper = static_cast<KCPWrapper*>(user);
    
    int sent = 0;
    if (!wrapper->mIsReceiver)
    {
        // 打印发送地址信息
        ...
        std::cout << "Sending to " << ip_str << ":" << ntohs(wrapper->mcast_addr_.sin_port) << std::endl;
         ...
         sent = sendto(wrapper->socket_fd_, buf, len, 0, 
    ...
    {
         sent = sendto(wrapper->socket_fd_, buf, len, 0, 
                        (struct sockaddr*)&wrapper->clientAddr, 
                        wrapper->client_addr_len);
    }
...
bool KCPWrapper::send(const char* data, size_t len) {
    ...
    std::cout << "KCPWrapper::send!" << std::endl;
...
void KCPWrapper::update() {
    ...
    //std::cout << "kcpwrapper update -> ikcp_update" << std::endl;
    // 更新KCP
...
void KCPWrapper::handleReceive() {
    while (true) {
        ...
        if (recv_len > 0)
            std::cout << "receive_len:" << recv_len << std::endl;
        ...
        // 输入数据到KCP
        auto rt  = ikcp_input(kcp_, recv_buffer_, recv_len);
        std::cout << "ikcp_input rt:" << rt << std::endl;

        // 从KCP接收数据
        while (true) {
            int kcp_recv_len = ikcp_recv(kcp_, recv_buffer_, BUFFER_SIZE);
            std::cout << "ikcp_recv length:" << kcp_recv_len << std::endl;
            if (kcp_recv_len < 0) {
                break;
            }
...
KCPWrapper::ErrorCode KCPWrapper::sendFileInternal(
    ...
    while (!file.eof()) 
    {
        ...
        std::cout << "sendFileInternal now update" << std::endl;\
